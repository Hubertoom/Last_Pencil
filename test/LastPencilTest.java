...
//Checking for compliance with the selected player
        if (!checkTurn) {
            throw new WrongAnswer("There should be a line in the output that contains the \""
...
//Checking, if conditions print, but the program then sends a request
        if (!main.isFinished()) {
            throw new WrongAnswer("Your program should not request anything after initial conditions have been printed.");
        }
        return CheckResult.correct();
    }
}