...
public class LastPencilTest extends StageTest<String> {
    ...
    CheckResult checkNumericAmount(String inp) {
        ...
        if (!output.contains("number of pencils") || !output.contains("numeric")) {
            throw new WrongAnswer("When the user provides the number of pencils as a non-numeric sequence, the game should " + "inform the user that their input is incorrect and prompt the user for input again" + " with the \"The number of pencils should be numeric\" string.");
        }
        for (int i = 1; i < 5; i++) {
            if (!output.contains("number of pencils") || !output.contains("numeric")) {
                throw new WrongAnswer("When the user provides the number of pencils as a non-numeric sequence, the game should " + "inform the user that their input is incorrect and prompt the user for input again" + " with the \"The number of pencils should be numeric\" string.");
    ...
    CheckResult checkNotZeroAmount() { //the test matches the name (checking positive)
        ...
        if (!output.contains("number of pencils") || !output.contains("positive")) {
            throw new WrongAnswer("When the user provides \"0\" as a number of pencils, the game should " + "inform the user that their input is incorrect and prompt the user for input again" + " with the \"The number of pencils should be positive\" string.");
        }
        for (int i = 1; i < 5; i++) {
    ...
    CheckResult checkBothAmount() {
        ...
        if (!output.contains("number of pencils") || !output.contains("numeric")) {
            throw new WrongAnswer("When the user provides the number of pencils as " +
...
// amount - number of pencils, first - player's turn, moves - number of operations, last - who was the last, incorrect - chars arrays for testing, winner - this winner ^_^
        ...
        if (!output.toLowerCase().contains("who") || !output.toLowerCase().contains("first")) { //Checking who first
            throw new WrongAnswer("The game should ask the user to input the player that goes first.");
        }
        // Getting names
        ...
        if (linesNonEmpty.size() != 2) { // checking a start task, program should print 2 non-empty lines
            throw new WrongAnswer("When the player provides the initial game conditions, your program should print 2 non-empty lines:\n"
        ...
        if (checkPencils.length == 0) { // checking initial game
            throw new WrongAnswer("When the player provides the initial game conditions, your program should print one line with several vertical bar symbols ('|') representing pencils.");
        }
        if (checkPencils.length > 1) { // checking if lines with pencils > 1
            throw new WrongAnswer("When the player provides the game initial conditions, your program should print only one line with several vertical bar symbols ('|') representing pencils.");
        }
        ...
        if (!lines[1].matches(prevPlayer + "|.+turn$")) { //checking player and task "turn"
            throw new WrongAnswer("When the player provides the correct initial game conditions"
                    ...
                    + nextPlayer + "'s turn that contains " + "\""
                    + nextPlayer + "\" and \"turn\" substrings if '"
...
//start checking task  possible values 1 2 3
        for (var i : moves) {
            for (String j : incorrect) {
                ...
                if (!output.contains("possible values") || !output.contains("1") || !output.contains("2") || !output.contains("3")) { // checking "Possible values: '1', '2', '3'" and re-entry requirement
                    throw new WrongAnswer("If the player enters values different from '1', '2', or '3'," + " the game should inform the user that " +
                ...
                if (!output.toLowerCase().contains(prevPlayer.toLowerCase()) && output.toLowerCase().contains(nextPlayer.toLowerCase())) {// check in case the request for re-entry has not been received
                    throw new WrongAnswer("When " + prevPlayer + " provides values different from '1', '2', or '3'," + " you need to prompt "
        ...
        if (linesNonEmpty.size() != 2) {// checking after player choice "if != 2 nonempty lines"
            throw new WrongAnswer("When one of the players enters the number of pencils they want to remove," + " the program should print 2 non-empty lines.");
        }
        ...
        if (checkPencils.length == 0) {//checking pencils in lines
            throw new WrongAnswer("When one of the players enters the amount of pencils they want to remove, your program should print one line with vertical bar symbols ('|') representing pencils.");
        }
        if (checkPencils.length > 1) {//checking if lines with pencils > 1 after player choice
            throw new WrongAnswer("When one of the players enters the amount of pencils they want to remove" + ", your program should print only one line with vertical bar symbols ('|') representing pencils.");
        }
        if (checkPencils[0].chars().distinct().count() != 1) {//checking any chars except |
            throw new WrongAnswer("The line with pencils should not contain any symbols other than the '|' symbol.");
        }
        if (checkPencils[0].length() != onTable) {//checking pencils in lines after player choice
            throw new WrongAnswer("When one of the players enters the amount of pencils they want to remove, " + "the line with pencils should contain as many '|' symbols as there are pencils left.");
        }

        if (!lines[1].matches(nextPlayer + "|.+turn$")) { // add regex checking player and turn
            throw new WrongAnswer(String.format("When %s enters the amount of pencils they want to remove"
        ...
        if (!output.contains("too many") || !output.contains("pencils")) { // checking condition if player choice pencils  "1 > | < 3"
            throw new WrongAnswer("If the player enters the number of pencils that is greater than the current " + "number of pencils on the table, the game should inform the user that " + "their input is incorrect and prompt the user for input again " + "with the \"too many pencils\" string.");
        }
        ...
        if (linesNonEmpty.size() != 1 || !linesNonEmpty.get(0).toLowerCase().contains(winnerName.toLowerCase()) || (!linesNonEmpty.get(0).contains("win") && !linesNonEmpty.get(0).contains("won"))) {
            if (linesNonEmpty.size() >= 1) {// checking if winner choices not correct
                if (!linesNonEmpty.get(0).toLowerCase().contains(winnerName.toLowerCase()) && (linesNonEmpty.get(0).contains("win") || linesNonEmpty.get(0).contains("won"))) {
                    throw new WrongAnswer("Make sure you determined the winner of the game correctly.\n" + "The player who takes the last pencil loses the game.");
                }
